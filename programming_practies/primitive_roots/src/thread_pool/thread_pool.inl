...
inline std::mutex Thread_Pool::ThreadPool::unfulfilledTasksMutex = std::mutex();

inline std::mutex Thread_Pool::ThreadPool::completedTasksMutex = std::mutex();
...
inline const std::function<void(Thread_Pool::TaskQueue&, 
                                                                        ...
                                                                        ) 
{
    while(controler) {    
        ...
        while (!unfulfilledTasks.empty()) {
            is_working = true;
            std::shared_ptr<BaseTask> executableTask;
            {
                std::unique_lock<std::mutex> lock(unfulfilledTasksMutex);
                if (unfulfilledTasks.empty()) {
                    break;
                }
                executableTask = unfulfilledTasks.get();
            }
            executableTask->start();
            {
                std::unique_lock<std::mutex> lock(completedTasksMutex);
                completedTasks.insert({executableTask->getID(), executableTask});
            }
            is_working = false;
...
inline Thread_Pool::ThreadPool::ThreadPool(size_t threadCount) : threads(threadCount)
{
    threadsStatus.clear();
...
inline Thread_Pool::ThreadPool::~ThreadPool()
{
...
inline size_t Thread_Pool::ThreadPool::addTask(const Task<Result, TypeArgs...>& source)
{
...
// Запуск всех потоков
inline void Thread_Pool::ThreadPool::start() 
{
...
inline void Thread_Pool::ThreadPool::stop()
{
...
inline void Thread_Pool::ThreadPool::stop(const std::thread::id& sourceID)
{
...
inline bool Thread_Pool::ThreadPool::unfulfilledTasksEmty() const
{
    return this->unfulfilledTasks.empty();
}
...
template<typename Result, typename ...TypeArgs>
inline std::optional<Thread_Pool::Task<Result, TypeArgs...>> Thread_Pool::ThreadPool::getCompletedTask(size_t taskID)
{
